...
const StyledAccentButton = styled(ButtonBase)`
  ...
  ${({ disabled, theme }) =>
    ...
    css`
      ...
      &:hover {
        background-color: ${theme.colors.accentPrimary.darken(40).toString()};
      }
      &:active {
        background-color: ${theme.colors.accentPrimary.darken(70).toString()};
      }
      &:focus-visible {
        outline-color: ${theme.colors.accentPrimary.darken(20).toString()};
      }
    `};
`;
...
const ButtonAccent: React.ForwardRefRenderFunction<HTMLButtonElement, ButtonAccentProps> = (
  ...
  return (
    <StyledAccentButton {...restProps} ref={ref}>
      {children}
    </StyledAccentButton>
  );
};
