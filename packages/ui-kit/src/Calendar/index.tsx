...
const Calendar: React.FC<CalendarProps> = props => {
  ...
  }, [year, month, calendar, onDateChange]);
  ...
  const years = React.useMemo(() => {
    ...
    for (
      ...
      year >= calendar.minDate.getFullYear() && year <= calendar.maxDate.getFullYear();
      year++
    ) {
      y.push(year);
    }
  ...
  const monthes = React.useMemo(() => {
    ...
    for (let index = 0; index < 12; index++) {
      ...
      if (d.getTime() > calendar.minDate.getTime() && d.getTime() < calendar.maxDate.getTime()) {
        m.push(index);
      }
    }

    return m;
  }, [calendar.date, calendar.maxDate, calendar.minDate]);
  ...
  return (
    <CalendarPaper>
      <CalendarTopBar>
        ...
        <span>
          <ButtonBase type="button" onClick={event => setMonthMenuEl(event.currentTarget)}>
            {currentMonthLabel}
          </ButtonBase>
          ...
          <Menu
            ...
            items={monthes.map(monthIndex => {
              ...
              return {
                value: monthIndex,
                label: intl.format(
                  new Date(calendar.date.getFullYear(), monthIndex, 1, 0, 0, 0, 0),
                ),
              };
            })}
