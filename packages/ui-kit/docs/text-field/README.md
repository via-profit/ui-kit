...
### `requiredAsterisk`
Отображение звездочки над Label, указывающей на обязательность поля для ввода.
- Тип: `boolean`
- По умолчанию: `false`
- Обязательное: нет

### `label`
Текст метки (label) текстового поля.
...
- Тип: `Ref<HTMLInputElement>`
- По умолчанию: (не указано)
- Обязательное: нет

### `fullWidth`
Если `true`, компонент занимает всю доступную ширину.
- Тип: `boolean`
- По умолчанию: `false`
- Обязательное: нет
...
#### `overrides.Input`
Компонент нативного элемента `<input>`.
- Тип: `React.Component`
- По умолчанию: (не указано)

#### `overrides.InputWrapper`
Обертка вокруг нативного инпута и иконок.
- Тип: `React.Component`
- По умолчанию: (не указано)
